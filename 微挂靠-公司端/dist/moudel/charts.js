...
function drawPieText(series, opts, config, context, radius, center) {
    ...
    var seriesConvert = series.map(function (item) {
        ...
        // var text = item.format ? item.format(+item._proportion_.toFixed(2)) :util.toFixed(item._proportion_ * 100) + '%';
        var text1 = item.name;
        var text2 = item.data+'万';
        var color = item.color;
        return { arc: arc, text1: text1,text2:text2, color: color };
    });
    ...
    textObjectCollection.forEach(function (item) {
        ...
        context.beginPath();
        context.setFillStyle('#666666');
        context.fillText(item.text1, textStartX, textPosition.y -6);
        context.fillText(item.text2,textStartX, textPosition.y + 12);
        context.closePath();
        context.stroke();
...
function drawLegend(series, opts, config, context) {
    ...
    var startY = opts.height - config.legendHeight + 15;
